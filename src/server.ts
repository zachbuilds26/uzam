            ...
            text: JSON.stringify(
              {
                ...
                name: asset.name,
                symbol: asset.symbol,
                summary: `${asset.symbol} — ${asset.name}, ${asset.asset_type} by ${asset.issuer} tracking ${asset.underlying_asset} on ${asset.chains.join(", ")} (chain ${asset.chainIds.join(", ")}). Official docs: ${asset.official_website}`,
            ...
            text: JSON.stringify(
              {
                ...
                summary: `${asset.symbol} onchain (X Layer ${contract}): price ${price?.price ?? info?.price ?? "n/a"}, holders ${info?.holders ?? "n/a"}, top-10 ${advanced?.top10HoldPercent ?? "n/a"}%, liquidity ${info?.liquidity ?? "n/a"} (confidence ${confidence}).`,
                observations,
                missing,
                confidence,
                data_timestamp: dataTimestamp,
