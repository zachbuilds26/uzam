...
- OKX Onchain OS status for X Layer: Wallet ✓, Trade ✓, Market ✓, ChainIndex 196
- xStocks: 1:1 backed tokenized US stocks/ETFs, issued via Backed Finance network, redeemable for cash value
- Count: 682 tokens on X Layer per xStocks tokenlist (same as Ethereum)
- Test assets for MVP:
  1. AAPLx (Apple)
  2. TSLAx (Tesla)
  3. NVDAx (Nvidia)
  4. SPYx (S&P 500 ETF) for comparison test
...
For application write: "Researches xStocks RWA tokens on X Layer (196) via OKX Onchain OS, with Ethereum fallback."
...
UZAM /mcp
 |
 +-- registry.json (static list of 4 test assets + docs links)
...
2. `analyze_onchain(symbol)` -> via OKX adapter: price, supply, holders, top holders concentration, trading activity, data_timestamp. If OKX down, return partial + `missing: ["onchain"]`.
...
- MCP: `@modelcontextprotocol/sdk` (Streamable HTTP, NOT old SSE)
...
To install in `uzam` (run once):
```
npm init -y
npm i express zod dotenv @modelcontextprotocol/sdk
npm i -D typescript @types/node @types/express tsx
```
Estimated sizes:
- `typescript` ~75 MB unpacked
- `@modelcontextprotocol/sdk` ~8 MB + deps ~15 MB total
- `express` ~5 MB + deps ~8 MB total
- `zod` ~2 MB, `dotenv` <1 MB, `tsx` ~5 MB
- Total `node_modules` MVP: ~120-180 MB
- Build output `dist/`: <2 MB
...
Phase 2: `src/data/xlayer-assets.json` with 4 xStocks + `identify_asset`.
...
## 11. Next command to start Phase 1 (when you say go)
```
